...
class TextDataset(Dataset):
    def __init__(self, preprocessor, args, isevaluate, lang, file_path, block_size=512):
        ...
        if os.path.exists(cached_features_file):
            ...
            error = []
            self.examples = []
            logger.info("Creating features from dataset file at %s", os.path.join(file_path, prefix +
                                                                                  '.jsonl.gz'))
            ...
            if not isevaluate and local_rank == 0:
                for idx, example in enumerate(self.examples[:5]):
                    ...
                    logger.info("inp_ids: {}".format(self.preprocessor.tokenizer.convert_ids_to_tokens(example.input_ids)))
                    logger.info("gen_labels: {}".format(self.preprocessor.tokenizer.convert_ids_to_tokens(example.tgt_ids)))
    ...
    for _, batch in merge_dataset(train_dataloaders, [len(x) for x in train_datasets], 0.7):
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                ...
                results = evaluate(args, model, preprocessor)
    ...
    for eval_dataloader in eval_dataloaders:
        for idx, source_ids, labels in iter(eval_dataloader):
            ...
            for label in labels:
                ...
                tgtss.append(gold.replace('<java>', '').replace('<cs>','').strip())
            ...
            with torch.no_grad():
                ...
                for pred in preds:
                    ...
                    p.append(text.replace('<java>', '').replace('<cs>','').strip())
    predictions = []
