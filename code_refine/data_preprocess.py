...
class Data_Preprocessor:
    ...
    def inp2features(self, instance, lang, attack = None):
        inp_code = instance['buggy']
        tgt_code = instance['fixed']
        ...
        if attack == None:
            inp_ids = self.tokenize(tokens, lang = lang, max_len = 350, clean = True)
            tgt_ids = self.tokenize(tgt_tokens, lang=lang, max_len=350)
        else:
            if attack == 'joint':
                tinp_tokens, ttgt_tokens = self.joint_attack(tokens, tgt_tokens, assign_list, tgt_assignlist, if_list,
                                                             tgtif_list, lang)
            else:
                tinp_tokens, ttgt_tokens = self.add_deadcode(tokens, tgt_tokens, assign_list, tgt_assignlist, if_list, tgtif_list, lang, attack = attack)
            if tinp_tokens == None:
                return None, None
            inp_ids = self.tokenize(tinp_tokens, lang = lang, max_len=350)
            tgt_ids = self.tokenize(ttgt_tokens, lang=lang, max_len=350)
    ...
    def add_deadcode(self, code, tgt_code, exp_list, tgt_exp_list, if_list, tgt_if_list, lang, attack = 'random'):
