...
class T5_seq2seq(nn.Module):
    ...
    def __init__(self, model, config, args, beam_size=None, max_length=None, sos_id=None, eos_id=None, type = False):
        super(T5_seq2seq, self).__init__()
        self.args = args
        self.model = model
        self.config = config
    ...
    def forward(self, input_ids = None, tgt_ids = None):
        ...
        if tgt_ids is not None:
            ...
            source_mask = input_ids.ne(0)
            target_ids = tgt_ids
            target_mask = tgt_ids.ne(0)
            outs = self.model(input_ids=source_ids, attention_mask=source_mask, decoder_input_ids=target_ids,
                                 decoder_attention_mask=target_mask, return_dict=True)
            ...
            source_ids = input_ids
            source_mask = input_ids.ne(0)
            encoder_output = self.model.encoder(input_ids=source_ids, attention_mask=source_mask)[0].permute(
                    [1, 0, 2]).contiguous()
            preds = []
            ...
            for i in range(source_ids.shape[0]):
                ...
                for _ in range(self.max_length):
                    ...
                    outs = self.model.decoder(input_ids=input_ids, encoder_hidden_states=context,
                                        encoder_attention_mask=context_mask)
                    out = self.model.lm_head(outs[0][:, -1, :])
